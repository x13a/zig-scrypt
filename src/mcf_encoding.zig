...
const ScryptParams = scrypt.ScryptParams;
...
pub const McfEncoding = struct {
    ...
    fn parseParams(encoded: *const [14]u8) !ScryptParams {
    ...
    pub fn verify(allocator: *mem.Allocator, str: []const u8, password: []const u8) !void {
        ...
        const params = try McfEncoding.parseParams(str[0..14]);
        var salt = str[14..];
        ...
        McfEncoding.sliceEncode(32, &encoded_dk, dk);
    ...
    pub fn create(allocator: *mem.Allocator, params: ScryptParams, password: []const u8) ![pwhash_str_length]u8 {
        var salt_bin: [32]u8 = undefined;
        crypto.random.bytes(&salt_bin);
        var salt: [encodedLen(salt_bin.len)]u8 = undefined;
        McfEncoding.sliceEncode(32, &salt, salt_bin);
        ...
        McfEncoding.intEncode(str[3..4], params.log_n);
        McfEncoding.intEncode(str[4..9], params.r);
        McfEncoding.intEncode(str[9..14], params.p);
        mem.copy(u8, str[14..57], &salt);
        str[57] = '$';
        McfEncoding.sliceEncode(32, str[58..], dk);
        return str;
    }
};
...
test "password hashing (crypt format)" {
    ...
    const params = ScryptParams.fromLimits(524288, 16777216);
